...
describe('slack-send', () => {
  ...
  describe('using a bot token', () => {
    ...
    describe('happy path', () => {
      ...
      it('should accept a payloadFilePath and use it\'s content in the message', async () => {
        // Prepare
        fakeCore.getInput.withArgs('channel-id').returns('C123456');
        fakeCore.getInput.withArgs('payloadFilePath').returns('./test/resources/valid-payload.json');
    ...
    describe('sad path', () => {
      ...
      it('should fail if an invalid payloadFilePath is provided', async () => {
        // Prepare
        fakeCore.getInput.withArgs('channel-id').returns('C123456');
        fakeCore.getInput.withArgs('payloadFilePath').returns('non-existing-path.json');

        // Run
        await slackSend(fakeCore);

        // Assert
        assert.include(fakeCore.setFailed.lastCall.firstArg.message, 'Need to provide valid payload file path', 'Error set specifying JSON was invalid.');
      });

      it('should fail if a valid payloadFilePath with an invalid JSON is provided', async () => {
        // Prepare
        fakeCore.getInput.withArgs('channel-id').returns('C123456');
        fakeCore.getInput.withArgs('payloadFilePath').returns('./test/resources/invalid-payload.json');
      ...
      it('should fail if inputs are missing', async () => {
        ...
        // Assert
        assert.include(fakeCore.setFailed.lastCall.firstArg.message, 'Missing input! Message not sent', 'Error set specifying JSON was invalid.');
      });
    });
  });
