import { IncomingWebhook, retryPolicies, WebhookTrigger } from "@slack/webhook";
    ...
    try {
      switch (config.inputs.webhookType) {
        case "incoming-webhook": {
          const response = await new IncomingWebhook(url, options).send(
        ...
        case "webhook-trigger": {
          const response = await new WebhookTrigger(url, options).send(
            config.content.values,
          );
  ...
  retries(option) {
    switch (option?.trim().toUpperCase()) {
      case "0":
        return { retries: 0 };
      case "5":
        return retryPolicies.fiveRetriesInFiveMinutes;
      case "10":
        return retryPolicies.tenRetriesInAboutThirtyMinutes;
      case "RAPID":
        return retryPolicies.rapidRetryPolicy;
      default:
        return retryPolicies.fiveRetriesInFiveMinutes;
    }
  }
